...
echo "TRAIN_DONE $OUT"
# window official test via online model
...
model=PredictiveUNet(
    ...
    embed_dim=int(ck.get("embed_dim",256)), mask_ratio=float(ck.get("mask_ratio",0.4)),
    use_residual=bool(ck.get("use_residual", True)), use_pred_in_forward=True,
)
...
Path("$W/test_${ARM}_s${SEED}_window.json").write_text(json.dumps({
  "arms":{"P":{"per_organ":per}},
  "intensity_mode":"window","arm":"$ARM","seed":$SEED,
  "note":"PredictiveUNet inference graph includes predictor"
}, indent=2))
